...
class Store:
        ...
        with self._connect() as conn:
            ...
            for item in payload.get("data", []):
                ...
                conn.execute(
                    ...
                    VALUES(?,?,?,?,?,?,?,?,?)""",
                    (handle, item.get("id"), attrs.get("asset_type", ""), attrs.get("asset_identifier", ""),
                     int(bool(attrs.get("eligible_for_bounty", False))), int(bool(attrs.get("eligible_for_submission", True))),
    ...
    def create_finding(self, data: dict[str, Any]) -> int:
        with self._connect() as conn:
            cur = conn.execute(
                ...
                VALUES(?,?,?,?,?,?,?,?,?,?,?,?)""",
                (data["program_handle"], data["target"], data["title"], data.get("severity"), data.get("state", "needs_review"),
                 data.get("summary", ""), data.get("impact", ""), json.dumps(data.get("reproduction", [])),
                 json.dumps(data.get("evidence", [])), data.get("structured_scope_id"), data.get("weakness_id"), utc_now()),
            )
    ...
    def set_state(self, finding_id: int, state: str) -> None:
        with self._connect() as conn:
            if state == "approved":
                conn.execute("UPDATE findings SET state=?, approved_at=? WHERE id=?", (state, utc_now(), finding_id))
            else:
